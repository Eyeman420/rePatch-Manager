...
-- THIS CODES IS USE FOR ENABLE AND DISABLE GAME THAT USING REPATCH
...
function Check_Folder(directory)
	if System.doesDirExist(directory) then
		...
		System.setMessage("rePatch folder not found in ux0:/rePatch", false, BUTTON_OK)
		while true do
			Graphics.initBlend()
...
-- Print message either enabled or disabled
function Print_Message(status)
	local message
	if status == true then
		message = "Sucessfully enabled rePatch game"
	else
		message = "Sucessfully disabled rePatch game"
	end
...
while folder_status == true do
	...
	-- Check inside folder
	if #folder == 0 then
		System.setMessage("No game folder found inside rePatch folder", false, BUTTON_OK)
		while true do
			Graphics.initBlend()
			status = System.getMessageState()
			if status ~= RUNNING then
		...
		Graphics.debugPrint(20, 0, "Game", Orange)
