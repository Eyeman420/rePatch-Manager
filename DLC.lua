...
-- THIS CODES IS USE FOR ENABLE AND DISABLE DLC THAT USING REPATCH
...
function Check_Folder(directory)
	if System.doesDirExist(directory) then
		...
		System.setMessage("reAddcont folder not found in ux0:/reAddcont", false, BUTTON_OK)
		while true do
			Graphics.initBlend()
...
function Print_Message(status)
	local message
	if status == true then
		message = "Sucessfully enabled reAddcont game"
	else
		message = "Sucessfully disabled reAddcont game"
	end
...
function Load_Game_List(game_list)
	...
	if System.doesFileExist("ux0:/data/rePatch-Manager/gamelist.txt") then
		...
		System.setMessage("gamelist.txt not exist in ux0:/", false, BUTTON_OK)
		while true do
			Graphics.initBlend()
			status = System.getMessageState()
			if status ~= RUNNING then
...
-- Variable for reAddcont directory
local dir = "ux0:/reAddcont"
...
-- Check folder
local folder_status
folder_status = Check_Folder(dir)

-- Main loop
while folder_status == true do

	-- reAddcont directory table
	local folder = System.listDirectory(dir .. "/")

	-- Check inside folder
	if #folder == 0 then
		System.setMessage("No game folder found inside reAddcont folder", false, BUTTON_OK)
		while true do
			Graphics.initBlend()
			status = System.getMessageState()
			if status ~= RUNNING then
		...
		Graphics.debugPrint(20, 0, "DLC", Orange)
