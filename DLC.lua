...
function CheckFolder(directory)
	if System.doesDirExist(directory) then
		...
		System.setMessage("Not found folder reAddcont in ux0:/reAddcont", false, BUTTON_OK)
		while true do
			Graphics.initBlend()
...
function Print_Message(status)
	...
	local message
	if status == true then
		message = "Sucessfully enable rePatch dlc"
	else
		message = "Sucessfully disable rePatch dlc"
	end
