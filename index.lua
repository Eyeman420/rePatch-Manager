...
-- List rePatch directory
local folder = System.listDirectory("ux0:/rePatch/")
local cfolder = "ux0:/rePatch/"
...
while true do
	...
	Graphics.fillRect(300, 920, 20, 100, grey) -- For game name
	...
	local found = false
	local specificFolder = string.gsub(folder[idx].name, "DIS", "")
	local gameTitle = "Not found" -- default message if not found
	local gidx = 1
	while found == false do
		if gidx == (#gameList + 1) then -- failed to find game name
			Graphics.debugPrint(310, 60, gameTitle, white)
			found = true
		elseif string.find(specificFolder, gameList[gidx].id) then
			gameTitle = gameList[gidx].name
			Graphics.debugPrint(310, 60, gameTitle, white)
			found = true
		end
		gidx = gidx + 1
	...
	if Controls.check(pad, SCE_CTRL_CROSS) and not Controls.check(oldpad, SCE_CTRL_CROSS) then -- Disable file
		if string.find(folder[idx].name, "DIS") then -- Avoid redundant rename folder
			...
			cfolder = cfolder .. folder[idx].name
			System.rename(cfolder, cfolder .. "DIS") -- rename from GameID to GameIDDIS
	...
	elseif Controls.check(pad, SCE_CTRL_CIRCLE) and not Controls.check(oldpad, SCE_CTRL_CIRCLE) then -- Enable file
		if string.find(folder[idx].name, "DIS") then -- Avoid redundant rename folder
			cfolder = cfolder .. folder[idx].name
			local newFolder = string.gsub(cfolder, "DIS", "") -- remove "DIS"
			System.rename(cfolder, newFolder) -- rename from GameIDDIS to GameID
